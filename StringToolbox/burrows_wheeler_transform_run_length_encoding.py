...
class BWTRLE(MethodModel):
    ...
    the lengths of compressed original strings. burrows and wheeler transform is
    a method for improvement on the compression.
    ...
    def _compressor(self, string):
        string += '\0'
        iterations = sorted([string[i:]+string[:i] for i in range(len(string))])
        string_transformed = ''.join([iteration[-1] for iteration in iterations])

        string_compressed = []
        for char, l in groupby(string_transformed):
            num = len(list(l))
            if num>2:
